...
const SmallScreenMenu = ({ isOpen, onClose }) => {
    ...
    const onOpenTeam = async (e) => {
        try {
            const tId = e.target.closest('span').getAttribute('data-team-id');
            const firstChannelId = await getFirstChannelIdByTeamId(tId);
            const chatRoomIdSnapshot = await getChatRoomIdByChannelId(firstChannelId);
            const chatRoomId = chatRoomIdSnapshot.val();
            navigate(`/teams/${tId}/${firstChannelId}/${chatRoomId}`);
        } catch (error) {
    ...
    const showTeamsOrChannels = () => {
        ...
        return <Grid templateColumns="repeat(auto-fit, minmax(calc(5em), 1fr))"
            justifyItems="start" alignContent="start" gap={6} h="100%" mt="2.5rem">
            <CreateTeam />
            ...
            <DrawerContent bg="primary" w="inherit">
                ...
                <DrawerHeader>
                    <Flex gap="1rem" justifyContent="center" alignItems="flex-start">
                        <IconButton id="msgs" bg="transparent" _hover={{ bg: 'transparent' }} onClick={handleClickOnMessages}
                        ...
                        <IconButton id="tms" bg="transparent" _hover={{ bg: 'transparent' }} onClick={handleClickOnTeams} className="open" icon={<HiOutlineUserGroup size="2.5em" />} />
                    </Flex>
                </DrawerHeader>
